...
int main(int argc, char*argv[])
{
    ...
    if(!strcmp(buffer, "Register failed")) exit(EXIT_FAILURE); // Register failed

    if((n = recv(sockfd, buffer, BUFSIZ, 0)) < 0) handle("recv"); // Recv hidden word
    buffer[n] = 0;
    word_lenght = strlen(buffer);   
    printf("\nSecret word's lenght: %d char\n\n", word_lenght);
    ...
    while(strstr(buffer, "_") && !strstr(buffer, "err: 6"))
    {
        ...
        if((send(sockfd, &c, 1, 0)) < 0) handle("send"); // Send a char
        if((n = recv(sockfd, buffer, BUFSIZ, 0)) < 0) handle("recv"); // Recv outcome
        buffer[n] = 0;        
        printf("%s\n", buffer);
    }
    if((n = recv(sockfd, buffer, BUFSIZ, 0)) < 0) handle("recv"); // Recv outcome
