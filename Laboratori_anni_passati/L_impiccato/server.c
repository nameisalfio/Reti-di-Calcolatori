...
bool Login(char* usr, char* ipv4, int port)
{
    FILE* fp;
...
char* Register(char* usr, char* ipv4, int port)
{
    if(Login(usr, ipv4, port)) return "Register failed";
...
char* choose_word(const char* filename) 
{
    ...
    while (fgets(word, WORD_SIZE, fp)) num_lines++;
...
bool handle_word(char* word, char* hidden_word, char c)
{
    if(!strchr(word, c)) return false;
...
int main(int argc, char*argv[])
{
    ...
    while(newsock = accept(sockfd, (struct sockaddr*)&client_addr, &len))
    {
        ...
        {
            close(sockfd);
            if((n = recv(newsock, buffer, BUFSIZ, 0)) < 0) handle("recv"); // Recv a char
            buffer[n] = 0;
            ...
            if((send(newsock, buffer, BUFSIZ, 0)) < 0) handle("send"); // Send outcome register        
            if((send(newsock, hidden_word, BUFSIZ, 0)) < 0) handle("send"); // Send hidden_word

            int err = 0; char c;
            while(strcmp(hidden_word, word) && err < 6)
            {
                if((n = recv(newsock, &c, 1, 0)) < 0) handle("recv"); // Recv a char
                if(!handle_word(word, hidden_word, c)) err++;
                sprintf(buffer, "Word: %s | err: %d\n", hidden_word, err);
                printf("%s\n", buffer);
                if((send(newsock, buffer, BUFSIZ, 0)) < 0) handle("send"); // Send result
            }
            ...
            if((send(newsock, buffer, BUFSIZ, 0)) < 0) handle("send");
            free(word);
            exit(EXIT_SUCCESS);
        }
        close(newsock);
